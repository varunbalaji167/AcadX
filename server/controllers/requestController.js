...
//       // Update sustainabilityScore for item owner
//       const owner = await User.findById(request.item.user);
//       if (owner) {
//         owner.sustainabilityScore += 10; // example: each completed item = 10 points
//         await owner.save();
//       }
//     }
...
//     const requestsWithRatings = await Promise.all(requests.map(async (req) => {
  ...
  try {
    ...
    const requests = await Request.find()
      .populate({
        path: "item",
        match: { user: userId },
        populate: { path: "user", select: "email name" }, // Include item owner info
      })
      .populate("requester", "email name"); // Also get requester name
  ...
  try {
    const requests = await Request.find({ requester: req.user._id })
      .populate({
        ...
        populate: {
          path: "user",
          select: "email name", // Include both email and name of item owner
        },
      });
