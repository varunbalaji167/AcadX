...
// const User = require("../models/userModel"); // ⬅️ Import your User model
...
//     // Step 2: Ensure the item belongs to the person being rated
//     if (item.user.toString() !== toUserId.toString()) {
//       return res.status(400).json({ success: false, message: "This item was not owned by the user you're trying to rate." });
//     }

//     // Step 3: Check for accepted request
...
//     if (!completedRequest) {
//       return res.status(400).json({ success: false, message: "No accepted request found for this item." });
//     }
...
//     if (existingRating) {
//       return res.status(400).json({ success: false, message: "You have already rated this user for this item." });
//     }

//     // Step 5: Save rating to Rating collection
...
//     // ✅ Step 6: Push rating into the `ratingsReceived` of the user
...
//     // ✅ Step 8: Mark the request as rated
...
//     const ratings = await Rating.find({ toUser: userId }).populate("fromUser", "email");
